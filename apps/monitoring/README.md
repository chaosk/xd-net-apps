...
| `dashboards/` | CNPG, Cilium, Spegel, Envoy Gateway, Speedtest Tracker, PeaNUT, and UniFi Poller Grafana dashboards (ConfigMaps for sidecar). |
...
| Spegel | Spegel Helm (**xd-net**) | Chart `serviceMonitor` on Service `spegel` port `metrics` (9090); label `release: kube-prometheus-stack`. |
...
| **Platform** | **Spegel** | Chart [`grafana-dashboard.json`](https://github.com/spegel-org/spegel/blob/main/charts/spegel/monitoring/grafana-dashboard.json) as `dashboards/spegel.json` ([Grafana 22104](https://grafana.com/grafana/dashboards/22104-spegel-monitoring-dashboard/)); `${DS_PROMETHEUS}` rewritten for sidecar | Spegel chart `serviceMonitor` (**xd-net**) |
...
JSON dashboards live under `dashboards/`; Kustomize builds ConfigMaps with label **`grafana_dashboard=1`** for the Grafana sidecar. Datasource placeholders are rewritten to **`Prometheus`** / **`${datasource}`**. Spegel’s upstream chart JSON cannot be provisioned as-is (`${DS_PROMETHEUS}` grafana.com placeholders), so we ship a rewritten copy instead of enabling the chart’s `grafanaDashboard`.
...
| **VpnGatewayDown** | `rules-vpn.yaml` + Gluetun exec readiness in `apps/vpn-gateway` | Gateway has zero available replicas (pod down **or** `/gluetun-entrypoint healthcheck` failing), `for: 2m` |
...
| vpn-gateway / Gluetun | kube-state-metrics available replicas; Gluetun readinessProbe runs `/gluetun-entrypoint healthcheck` in-container (not kubelet httpGet — Gluetun firewall times out node→pod `:9999`) | Catches the common Gluetun “restarting VPN because healthcheck failed” loop. Still not a full egress IP assertion (e.g. ipify from a routed pod). |
